...
- [ ] `git mv tests/test_*.py tests/unit/` (add `tests/unit/__init__.py`)
- [ ] `git mv tests/api tests/unit/api`
- [ ] Confirm pytest collects both trees (`testpaths = ["tests"]`)

## REQ-TI-002 — Unit tests behavior parity

- [ ] Run `uv run pytest tests/unit/ -q` — all previously passing tests pass
- [ ] Run `uv run pytest tests/unit/ --cov=engagedin --cov=cli --cov=api
      --cov-fail-under=100 -q` — gate holds from unit tests alone

## REQ-TI-003 — Test database service

- [ ] Create `docker-compose.test.yml` (postgres:16-alpine, port 5433,
      `engagedin_test`, healthcheck, named volume)
- [ ] Confirm dev `docker-compose.yml` untouched

## REQ-TI-004 — Session-scoped integration engine

- [ ] Add `_get_test_db_url()` helper (`POSTGRES_PORT` default 5433) to
      `tests/conftest.py`
- [ ] Add session-scoped `integration_db_engine` fixture: unique
...
- [ ] Mock only at the `Engine` boundary with `@patch` decorators

## REQ-TI-009 — API-flow integration coverage

- [ ] Write `tests/integration/test_api_flow.py`: healthz reachable, full
...
- [ ] Verify `make test`-style run (`uv run pytest tests/`) completes with
      integration tests skipped when Postgres is stopped

## REQ-TI-011 — Help target

- [ ] Create `Makefile` with `help` target listing all commands

## REQ-TI-012 — Quality targets

- [ ] Add `format` (`ruff check --fix .`), `lint` (`ruff check .`),
      `typecheck` (`mypy engagedin cli api`) with failure guards

## REQ-TI-013 — Unit test targets

- [ ] Add `unit-test` (`pytest tests/unit/ -vv -n auto`)
- [ ] Add `coverage` (`pytest tests/unit/ --cov=engagedin --cov=cli --cov=api
      --cov-fail-under=100 -n auto`)
- [ ] Add `test` (full suite, serial, `--tb=short`)

## REQ-TI-014 — Integration database up target

- [ ] Add `integration-test-up`: compose up → `pg_isready` retry loop (10×2s)
      → `alembic upgrade head` against `…:5433/engagedin_test`

## REQ-TI-015 — Integration database down target

- [ ] Add `integration-test-down` (`docker compose -f
      docker-compose.test.yml down -v`)

## REQ-TI-016 — Integration test target

- [ ] Add `_NPROC` / `INTEGRATION_WORKERS` (capped at 8) variables
- [ ] Add `integration-test` target: up → `pytest tests/integration/ -v -n
      $(INTEGRATION_WORKERS) --dist loadscope` → down; down also on failure

## REQ-TI-017 — Run target

- [ ] Add `run`: dev compose up + readiness loop +
      `uvicorn api.main:app --reload`

## REQ-TI-018 — pytest-xdist dependency

- [ ] Add `pytest-xdist (>=3.6)` to the `dev` dependency group
- [ ] `uv lock` / `uv sync` and confirm resolution
...
- [ ] Confirm session-scoped async fixtures work under `-n auto`

## REQ-TI-020 — Parallel CI gates

- [ ] Update CI: unit step (`tests/unit/` with coverage gate, `-n auto`, no
      DB dependency) and integration step (`tests/integration/ -n 4 --dist
      loadscope`)
- [ ] Point the Postgres service at host port `5433` with database
      `engagedin_test`; update `DATABASE_URL` env accordingly
- [ ] Confirm Python matrix and 100% coverage gate preserved

## Verification

- [ ] `make format && make lint && make typecheck` pass
- [ ] `make unit-test` passes (parallel, no DB running)
- [ ] `make integration-test` passes and cleans up (run twice)
- [ ] `make coverage` passes with 100% gate (no DB running)
- [ ] `uv run pytest tests/` with Docker stopped: integration skipped, exit 0

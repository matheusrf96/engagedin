...
- [ ] Create `engagedin/core/languages.py` with `LANGUAGE_TAG_RE`,
      `resolve_language` (trim, normalize primary/subtag case, validate,
      `ValueError` on invalid)
- [ ] Unit tests in `tests/unit/test_languages.py`: `"en"`, `"PT-br"` →
      `pt-BR`, `"zh-hans"` → `zh-Hans`; `""`, `"not a tag!"` raise `ValueError`

## REQ-ML-002 — Display names

- [ ] Add `LANGUAGE_NAMES`, `LANGUAGE_OVERRIDES`, and
      `language_display_name` (full tag → primary → raw tag fallback)
- [ ] Unit tests: `pt-BR` → "Brazilian Portuguese", `zh-Hans` → "Simplified
      Chinese", `ar` → "Arabic", unknown tag → raw tag

## REQ-ML-003 — Caseless-script detection

- [ ] Add `CASELESS_LANGUAGES` and `has_case` (primary-subtag decision)
- [ ] Unit tests: `False` for `zh`, `ar`, `ja`, `he`; `True` for `en`, `ru`,
      `pt-BR`

## REQ-ML-004 — Language on the ruleset

- [ ] Add `language: str = "en"` to `PostRuleset` in
      `engagedin/core/models.py`
- [ ] `engagedin/rules/loader.py` reads `language` via `resolve_language`
- [ ] Add `language: en` to `engagedin/rules/defaults.yaml`
- [ ] Unit tests in `tests/unit/test_models.py` (default) and
      `tests/unit/test_rules.py` (YAML override, absent key, invalid value
      raises)

## REQ-ML-005 — Output-language instruction

- [ ] `engagedin/llm/prompts.py`: add `- Write the entire post in {language}`
      ...
      `hashtag_rule=...`
- [ ] Unit tests in `tests/unit/test_llm.py`: prompt contains "Russian" for a
...
- [ ] `HEADLINER_USER_PROMPT`: require the final `SOURCE: <number>` line in
      exact ASCII format regardless of post language
- [ ] `SOURCE_LINE_RE` in `engagedin/core/engine.py` accepts `:` and `：`
- [ ] Unit tests: regex matches `SOURCE：3`; built user prompt contains the
      ASCII instruction

## REQ-ML-007 — Per-call language override

- [ ] `Engine._effective_ruleset(language)` (`model_copy`, no mutation) and
      `language: str | None = None` parameters on `generate_draft` and
      `generate_headliner_draft`
- [ ] Unit tests in `tests/unit/test_engine.py`: LLM receives ruleset with
      resolved language; `self.ruleset` unmutated; `None` passes ruleset
      as-is

## REQ-ML-008 — CLI `--language` option

- [ ] `cli/main.py`: `--language/-l` on `draft`, `post`, `headliner`; resolve
      up front, `_fail` on `ValueError`; resolved tag in the panel title
- [ ] Unit tests in `tests/unit/test_cli.py`: `--language ru` reaches the
      engine; invalid tag exits 1 before generation; omitted → unchanged

## REQ-ML-009 — API request field and validation

- [ ] `api/schemas.py`: `DraftCreateRequest.language: str | None = None`
- [ ] `api/services/posts.py`: `create_draft` resolves the tag, raises
      `ExternalServiceError(status_code=400)` on `ValueError`, forwards the
      resolved value through `_generate` to the engine
- [ ] `api/routers/generation.py`: pass `request.language`
- [ ] Unit tests in `tests/unit/api/test_service.py` and
      `test_generation.py`: resolved value reaches the engine; invalid tag →
      400; omitted → unchanged behavior

## REQ-ML-010 — Persistence

- [ ] `api/models.py`: `language: Mapped[str | None] = mapped_column(String(12))`
      on `PostRecord`; `create_draft` stores resolved tag or `NULL`
- [ ] Create `migrations/versions/0003_post_language.py` (`down_revision`
      `0002`) with symmetric downgrade
- [ ] Unit tests: record carries the resolved language / `None`
...
- [ ] `api/schemas.py`: `PostOut.language: str | None`
- [ ] Unit tests in `tests/unit/api/test_generation.py` / `test_posts.py`
      asserting the field appears in responses
- [ ] Update `tests/integration/test_api_flow.py` for the new response field

## REQ-ML-012 — Non-Latin end-to-end

- [ ] Audit the changed paths for encoding assumptions (no `.encode()` /
      `.decode()`, no ASCII-only regex on post content); confirm
      `character_count` stays `len(content)`
- [ ] Unit tests with non-Latin fixtures: generated content in Arabic /
...
- [ ] `uv run mypy .` passes (pre-existing test-file errors unaffected)
- [ ] `uv run pytest` passes with the existing coverage gate
- [ ] `docker compose up -d` + `uv run alembic upgrade head` + empty
      `alembic revision --autogenerate` diff
- [ ] Smoke: `uv run engagedin draft "AI regulation" --language zh-Hans` and

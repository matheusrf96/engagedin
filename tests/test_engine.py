...
class TestEngine:
    ...
    def test_publish_draft_with_urn(self) -> None:
        mock_linkedin = MagicMock(spec=LinkedInClient)
        ...
        with patch(
            "engagedin.core.engine.settings"
        ) as mock_settings:
    ...
    def test_publish_draft_resolves_urn(self) -> None:
        mock_linkedin = MagicMock(spec=LinkedInClient)
        mock_linkedin.create_post.return_value = "urn:li:share:12345"
    ...
    def test_generate_and_publish(self) -> None:
        mock_llm = MagicMock(spec=LLMClient)
